...
将class字节码文件加载到JVM，并存储在运行时内存区的方法区。（并将这些数据转换成方法区中的运行时数据（静态变量、静态代码块、常量池等），在堆中生成一个Class类对象代表这个类（反射原理），作为方法区类数据的访问入口。）
 ...
 **• 准备**
			 **为类的static变量分配内存并设置类变量初始值（隐式初始化）**的阶段，这些内存都将在方法区中进行分配。注意此时的设置初始值为默认值，具体赋值在初始化阶段完成。
...
**①定义：<clinit>()方法是由编译器自动收集类中的所有的静态变量的赋值和静态代码块语句合并产生的，没有的话则不会产生。** 
  ...
  **c.Java虚拟机必须保证一个类的**<clinit>()方法**在多线程的环境中被同步加锁。如果多个线程同时去初始化一个类，必须只能其中一个去执行这个类**<clinit>()方法。
